...
{
    abstract class CollisionBaseXT : VFXBlock
    {
        public enum Mode
        {
            Solid,
        ...
        [VFXSetting, Tooltip("When enabled, Records collision to be used with a TriggerOnCollision (XT) Block")]
        public bool recordCollision = true;
        ...
        public override IEnumerable<VFXAttributeInfo> attributes
        {
            get
            {
                ...
                if(recordCollision)
                {
                    yield return new VFXAttributeInfo(TriggerOnCollisionXT.collisionChannel, VFXAttributeMode.ReadWrite);
                    yield return new VFXAttributeInfo(TriggerOnCollisionXT.collisionPosition, VFXAttributeMode.Write);
                    yield return new VFXAttributeInfo(TriggerOnCollisionXT.collisionDirection, VFXAttributeMode.Write);
                }
            }
        }
        ...
        protected override IEnumerable<VFXPropertyWithValue> inputProperties
        {
            get
            {
                ...
                if (recordCollision)
                    properties = properties.Concat(PropertiesFromType("CollisionEventProperties"));
...
";
        ...
        protected string collisionResponseSource
        {
            get
            {
                ...
                string collisionAtt = "";
                if (recordCollision)
                    collisionAtt = @"        collisionChannel = CollisionChannel;
...
";
        ...
        public class CollisionEventProperties
        {
            [Min(0), Tooltip("Sets the Collision Channel Attribute (to differentiate the collision source in GPUEvent)")]
